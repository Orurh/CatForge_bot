# Архив — не спецификация для реализации

Исторический план до beta hardening 07.09.2026. Содержит отменённые решения и старые числа. Единственный актуальный индекс: [PLAN.md](../../PLAN.md). Этот архив не задаёт требований к коду.

> Актуализация 07.09.2026: формула XP, порог 50, полный расход энергии и крит тренировки описаны в [Training v2](docs/TRAINING_V2.md); прежние цифры ниже исторические.

# CatForge v3 — Telegram-native план

> Feline Progression v1 реализован в rules 12 / content 5. Актуальные формулы,
> открытия, предметы, миграция и проверки описаны в
> [docs/FELINE_PROGRESSION.md](docs/FELINE_PROGRESSION.md).
> Старые RPG-примеры и описания скрытого Equipment ниже относятся к предыдущему MVP;
> для прогрессии актуален связанный документ. Недельные звания — раздел 87.
...
и ограничен одним показом своего кота в каждом групповом чате за календарный день.
...
После результата новую драку сразу начать нельзя: действует общий лимит одной драки на кота за 24 часа.

---

# 45. Реванш пока скрыт

При лимите одной драки на кота за 24 часа мгновенный реванш противоречит правилу.
Кнопку не показываем; backend-код можно оставить как legacy до нового решения.

---

# 46. Небольшой XP за драки без фарма

Лимит одной драки на кота за 24 часа уже закрывает фарм. За участие начисляется
3% порога XP текущего уровня, за победу — ещё 2%.

Fight также сохраняет win/loss, текущую серию, rivalry и social history; эти raw
метрики используются в `/profile`, недельном счёте и званиях.
...
```text
1 fight / cat / rolling 24 hours
```

Лимит глобальный для кота: переход в другой Двор не сбрасывает 24-часовое
ограничение.
...
Так одна пара не может заспамить чат, а каждый бой остаётся редким событием.
...
Это финал текущего недельного цикла и видимая команда группы.

Каждая категория даёт максимум 7 очков: один бой — одно Arena‑очко,
участие в доступных событиях нормализуется до 7 Yard‑очков, а каждые 90 единиц
потраченной энергии дают одно Training‑очко. `/week` показывает TOP‑10 с
максимумом 21, raw W/L, энергию, участие, MVP и ровно одно приоритетное звание
на кота (если выполнено условие звания). Training считается глобально для кота:
личные тренировки и тренировки в других группах тоже учитываются. Arena и Yard
относятся к текущему Двору. Для Yard знаменатель персональный: учитываются
разрешённые события недели, до окончания которых кот успел вступить в Двор.

Звания в порядке приоритета:

* 🗿 Сигма-кот — первое место по Total;
* 🥲 Пакет для битья — семь боёв без побед;
* 🏆 Всё на мне, мяу — максимум MVP;
* 😾 Ты с какого лотка? — максимум прироста rivalry;
* 🏋️ Шкаф с усами — максимум потраченной Training energy;
* 🥊 Лапами объясню — семь боёв;
* 🏘 В каждой бочке кот — участие во всех доступных событиях, минимум в трёх;
* 🛋 Я чисто посмотреть — минимум Total среди котов с реальными действиями.

Каждое звание выдаётся максимум одному коту. Рекордные показатели сравниваются
со всеми котами, даже уже получившими звание: второе место не становится
рекордсменом автоматически. При равном рекорде выбирается меньший CatID.
Условия «семь боёв» и «все события» выбирают первого подходящего кота без звания
в отсортированном рейтинге. Старое звание «На опыте» больше не выдаётся.
...
# 128. Очень важная метрика — return rate

После Fight и окончания 24-часового лимита:

> сколько участников снова вышли на арену?
...
6. legacy revenge backend без публичной кнопки;
7. fight cooldown;
8. win/loss;
9. rivalry;
...
После боя оба кота достигают лимита на 24 часа; кнопки реванша нет.
...
Фазы 0–4 закрыты в коде: есть idle-перепалки раз в 24–48 часов,
парные реакции на заметные Yard Events, strict daily limits,
procedural fallback и админские переключатели реплик, banter, humor, quiet и fights.
Обычные тренировки остаются procedural; AI вызывается лишь для
level-up, critical result и rivalry-контекста.

Остался главный этап: недельный тест с реальными группами и разбор метрик.
...
* Event participation;
* fights;
* return to arena после 24-часового лимита;
* replies котам;
* banter reactions;
* disable rate.
...
> «Барсик его завтра на арене дождётся.»
...
* Yard Event;
* Fight;
* arena queue и 24-часовой cooldown;
* relationships.
